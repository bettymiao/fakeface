...
class GRL(torch.autograd.Function):
    @staticmethod
    def forward(ctx, input, iter_num, alpha, low, high, max_iter):
        iter_num += 1
...
class Discriminator(nn.Module):
    def __init__(self, in_features, num_classes):
        ...
        self.iter_num = torch.tensor(0)
        self.alpha = torch.tensor(10)
        self.low = torch.tensor(0.0)
        self.high = torch.tensor(1.0)
        self.max_iter = torch.tensor(4000)  # be same to the max_iter of config.py
        
    def forward(self, feature):
        out = self.grl_layer(feature, self.iter_num, self.alpha, self.low, self.high, self.max_iter)
        adversarial_out = self.ad_net(out)
