...
def getLossFunc(device, cfg):
    ...
    elif 'NewOneCenterLoss' in cfg['loss']:
        loss_func = NewOneCenterLoss(cfg['class_number']).to(device)

    elif 'OneCenter' in cfg['loss']:
        loss_func = OneCenter(cfg['class_number'], cfg["foc_weight"]).to(device)

    elif 'CenterLoss' in cfg['loss']:
        center_weight = float(cfg['loss'].strip().split('-')[1])
        loss_func = CEandCenter(cfg['class_number'], 2,
            size_average=True, label_smooth=0, 
            class_weight=None, center_weight=center_weight).to(device)
