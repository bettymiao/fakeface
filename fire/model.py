...
from fire.DGFAS import Discriminator
import torchvision
...
class FireModel(nn.Module):
    ...
    def changeModelStructure(self):
        ...
        if not self.cfg["use_timm"]:
            ...
            # fc_features = self.pretrain_model.head.in_features
            fc_features = self.pretrain_model.num_features
            ...
            self.tanh = nn.Tanh()
            self.head2 = nn.Linear(feat_dim, self.cfg['class_number']) 
            self.head3 = nn.Linear(fc_features, self.cfg['class_number'])
        ...
        self.discriminator = Discriminator(512, self.cfg['class_number'])
    ...
    def forward(self, img):        
        if not self.cfg["use_timm"]:
            if "efficientnet" in self.cfg['model_name']:
                ...
                # 添加判别器, [64, 3]
                discriminator_out = self.discriminator(out)
                ...
                out = [out2, out1, discriminator_out]
            ...
            out = self.pretrain_model.forward_features(img)
            out = self.pretrain_model.forward_head(out, pre_logits=True)
            ...
            out2 = self.head2(out1)
            out = [out2, out1, feature]
            
            # out = [self.head3(out), out, discriminator_out]
        return out
